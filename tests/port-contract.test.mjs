...
const rec = JSON.parse(await readFile(new URL('../contracts/corpus/recommendation.json', import.meta.url)));
const valid = JSON.parse(await readFile(new URL('../contracts/corpus/snapshot.valid.json', import.meta.url)));
const defects = JSON.parse(await readFile(new URL('../contracts/corpus/known-defects.json', import.meta.url)));
const http = JSON.parse(await readFile(new URL('../contracts/corpus/http-guard.json', import.meta.url)));
...
test('typescript is a declared devDependency so typecheck cannot silently skip', () => {
  ...
  assert.equal(pkg.scripts['check:port'], 'node scripts/check-port.mjs');
});
