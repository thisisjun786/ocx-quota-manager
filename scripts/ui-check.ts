...
import { buildFixture, FIXTURE_VARIANTS } from './ui-fixture.ts';
import { openDashboard, SCREENS } from './ui-browser.ts';
import { regressionFlow } from './ui-flow.ts';
...
const failures: any[] = [];
const checks: any[] = [];
function expect(condition: unknown, description: string, detail?: unknown) {
...
for (const name of FIXTURE_VARIANTS) {
  ...
  for (const p of world.providers) {
    ...
    for (const a of p.accounts) for (const w of a.windows) {
      ...
      // Every condition the server returns on before the forecast runs. A fixture holding a
    ...
    if (rec.status === 'provisional') {
      ...
      // An unanswered suggestion still reports what it did work out. The server
...
const { excludedNote } = await import('../webembed/static/quota.js');
...
const { unitPrice } = await import('../webembed/static/format.js');
...
// The session, the page helpers and the console collector live in ./ui-browser.ts so that
...
const { quotaFigure } = await import('../webembed/static/format.js');
...
expect(JSON.stringify(figureCases) === JSON.stringify(['28', '28.5', '12.35', '0', '0', '<0.01',
  ...
  figureCases);
const { directStatusNote, directStatusIsWarning, instantNote } = await import('../webembed/static/quota.js');
...
const { call, evaluate, settle, text, countOf, clickText, problems, publish } = page;
