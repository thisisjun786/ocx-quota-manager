...
import { PRICE_CONDITIONS } from '../src/pricing.mjs';
import { judgeModelPrice, PRICE_GAP_REASONS, priceGapKey } from '../src/price-gaps.mjs';
// Deterministic snapshots for the dashboard smoke check. They mirror the shape
// src/analytics.mjs produces and cover the states the UI must distinguish: fresh,
...
// exact integers. The two coverage figures follow src/analytics.mjs:283-296:
...
const HOUR = 3600000;
const PERIODS = [['oneHour', 1], ['fiveHour', 5], ['twentyFourHour', 24], ['weekly', 168], ['monthly', 720]];
...
export function buildFixture(variantName = 'default') {
  ...
  const span = hours => {
    const row = { startedAt: fromT(-hours * HOUR), endedAt: fromT(0), hours,
  ...
  // a missing usage report and a reported zero into the same 0 (src/history.mjs num()), so the
  ...
  // The account need keeps its own sample and follows src/recommendation.mjs:26-50:
  ...
  const recommendationOf = (periods, configured, eligible, fiveHourAccounts) => {
    ...
    // rows whose amount resolved (src/history.mjs:419). A provider holding nothing but
    ...
    // src/recommendation.mjs:23-35 decides in this order: a history, then a priced month,
  ...
  // A window src/analytics.mjs returns early for carries no forecast at all. Mirroring emptyResult
  ...
  const windowAnalytics = (overrides = {}) => legacyShape({ status: 'ok', recentRatePpHour: 0.8, averageRatePpHour: 0.6,
    ...
    ...overrides });
  // What the number is based on, mirroring src/quota-measurement.mjs. Every window carries one
  ...
  const quotaWindow = (id, label, usedPercent, overrides = {}) => ({ id, label, usedPercent,
    ...
    // providerWide mirrors what src/window-scope.mjs answers: the standard ids measure the
    ...
    analytics: windowAnalytics({
      ...
      // src/analytics.mjs returns before the forecast for a stale window, a missing reset and a
      ...
      ...(overrides.stale || !(Date.parse('resetAt' in overrides ? overrides.resetAt : at(6 * HOUR)) > now)
        ? NO_FORECAST : {}),
      ...overrides.analytics }) });
  const account = (id, label, periods, overrides = {}) => ({ id, label, plan: 'pro', active: true,
    status: 'ok', updatedAt: at(-30000), quotaMode: 'observed',
    refresh: { status: 'ok', lastAttemptAt: at(-30000), nextAttemptAt: null }, ...overrides,
    // src/analytics.mjs returns before the forecast for a reauth or paused account, so the windows
  ...
  const provider = (id, name, enabled, defaultModel, accounts, unattributed, unlisted, extra = {}) => {
  ...
  // 단가 값은 src/pricing.mjs 의 실제 표에서 확인한 것이고, 모양은 tests/collector.test.mjs 의
  ...
  const priceGapsFor = (id, prices, rosterModels) => {
    const models = [];
    for (const row of prices) {
      const judged = judgeModelPrice(row);
      if (!judged.length) continue;
      models.push({ model: row.model, sources: [...row.sources], priceStatus: row.status,
        pricedModel: row.pricedModel, requests: row.requests, unpricedRequests: row.unpricedRequests,
        tokens: row.tokens,
        rosterState: rosterModels.find(entry => entry.model === row.model)?.state ?? null,
        // 한 모델만 확인 성공 기록을 가진다. 나머지는 null 이고 화면이 그 둘을 갈라 적는다.
        lastConfirmedAt: row.model === 'gpt-5.4-mini' ? at(-2 * 86400000) : null,
        periods: gapPeriods(row.model), carriedFindings: [],
        findings: judged.map(finding => ({ key: priceGapKey(id, row.model, finding.reason),
          reason: finding.reason, note: PRICE_GAP_REASONS[finding.reason],
          detail: finding.detail ?? null, items: finding.items ?? null,
          pricedModel: finding.pricedModel ?? null, state: 'open',
          // 한 사유는 해결됐다가 다시 나타난 것으로 둔다.
          recurred: finding.reason === 'price-conflict',
          firstSeenAt: at(-6 * 86400000), lastSeenAt: at(-5000), occurrences: 30,
          recurrences: finding.reason === 'price-conflict' ? 2 : 0 })) });
    }
    models.sort((a, b) => b.requests - a.requests || a.model.localeCompare(b.model));
    // 집계는 위 목록의 합이다. 서버가 그렇게 만들기 때문이고, 그래야 검사가 둘의 어긋남을 잡는다.
    const periods = build((key, hours) => {
      const sum = pick => models.reduce((total, model) => total + pick(model.periods[key] ?? EMPTY_BUCKET), 0);
      return { activeModels: models.filter(model => (model.periods[key]?.requests ?? 0) > 0).length,
        requests: sum(bucket => bucket.requests), tokens: sum(bucket => bucket.tokens),
        unpricedRequests: sum(bucket => bucket.unpricedRequests),
        unpricedTokens: sum(bucket => bucket.unpricedTokens),
        unpricedUnsizedRequests: sum(bucket => bucket.unpricedUnsizedRequests),
        unnamedModelRequests: 0, unnamedModelUnpricedRequests: 0, unnamedModelUnpricedTokens: 0,
        withheldModelRequests: 0, withheldModelUnpricedRequests: 0, withheldModelUnpricedTokens: 0,
        startedAt: fromT(-hours * HOUR), endedAt: fromT(0), hours };
    });
    const resolvedKey = priceGapKey(id, 'gpt-6-astra', 'source-missing');
    return { status: 'ok', since: at(-20 * 86400000), readings: 240,
      modelsNeedingPriceCheck: models.length, models, carriedModels: [],
      // 해결은 건수에서 빠지고 이력에만 남는다. 어디로 갔는지 읽을 자리가 없으면 줄어든 숫자가
      // 그냥 사라진 것처럼 보인다.
      resolved: id === 'openai' ? [{ key: resolvedKey, model: 'gpt-6-astra', reason: 'source-missing',
        note: PRICE_GAP_REASONS['source-missing'], firstSeenAt: at(-12 * 86400000),
        lastSeenAt: at(-4 * 86400000), resolvedAt: at(-4 * 86400000), occurrences: 40, recurrences: 0 }] : [],
      changes: id === 'openai' ? [
        { at: at(-4 * 86400000), model: 'gpt-6-astra', reason: 'source-missing', change: 'resolved' },
        { at: at(-6 * 86400000), model: 'mystery-model', reason: 'price-missing', change: 'opened' }] : [],
      confirmedNewModels: [], retiredModels: [], periods };
  };
  const priceFacts = (id, models, evidence = []) => {
  ...
  if (!v.legacy) for (const p of providers) {
    p.analytics.recommendations = Object.fromEntries(Object.entries(p.analytics.periods).map(([key, stats]) => {
      const hours = stats.observedCoverageHours ?? 0;
      return [key, recommendSubscriptions(p, {pricedBounds:()=>({since:null}), peakFiveHour:()=>0}, now,
        {key, hours, periodHours:stats.hours, stats:{...stats,
          apiUsd:hours > 0 && stats.requests === 0 ? 0 : stats.apiUsd === null ? null : stats.apiUsd * hours / Math.max(hours, Math.min(stats.hours, retainedDays * 24))}})];
    }));
    for (const a of p.accounts) for (const w of a.windows) {
      const data = w.analytics;
      data.consumptionPeriods = Object.fromEntries(Object.entries(p.analytics.periods).map(([key, stats]) => {
  ...
  if (!v.legacy) for (const p of providers) p.analytics.quotaRecommendations = Object.fromEntries(
    Object.entries(p.analytics.periods).map(([key, stats]) => [key, recommendQuotaAccounts(p, key, stats.hours)]));
  ...
  return { schemaVersion: 1, observedAt: at(0), source: 'ui-check-fixture:' + variantName,
    ...
    analytics: { subscriptionMonthlyUsd: providers.reduce((total, p) => total + p.analytics.subscriptionMonthlyUsd, 0),
      ...
      // 조건 코드의 설명은 서버가 한곳에서 내려준다. 고정 문자열을 복제하면 코드와 설명이 갈린다.
      modelPriceConditions: PRICE_CONDITIONS,
