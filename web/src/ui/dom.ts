...
export function node<K extends keyof HTMLElementTagNameMap>(tag: K, className?: string, value?: string): HTMLElementTagNameMap[K] {
...
export function metric(label: string, value: string, note?: string | null): HTMLElement {
  const item = node('div', 'metric');
  item.append(node('span', '', label), node('strong', '', value));
  if (note) item.append(node('small', '', note));
  return item;
}
export function quotaTrack(remaining: unknown, label: string): HTMLElement {
...
function historyPoint(value: unknown): {at: string; usedPercent: number; resetAt?: string} | null {
  const o = asRecord(value);
  if (!o || typeof o.at !== 'string' || !finite(Date.parse(o.at))) return null;
...
export function historyChart(history: unknown, label: string): HTMLElement {
  const wrap = node('div', 'spark');
  const typed = Array.isArray(history) ? history.map(historyPoint).filter((p): p is NonNullable<typeof p> => p !== null) : [];
  if (typed.length < 2) {
...
export function sourceLink(source: {url?: string; label?: string; checkedAt?: string} | null | undefined): HTMLElement {
  const item = node('li');
  if (source?.url) {
    const a = node('a', '', source.label || source.url) as HTMLAnchorElement;
    a.href = source.url;
...
export function methodology(analytics: {notes?: string[]; sources?: {url?: string; label?: string; checkedAt?: string}[]} | null | undefined, extra?: {key?: string; title?: string; notes?: string[]; sources?: {url?: string; label?: string; checkedAt?: string}[]; body?: Node}): HTMLElement {
