...
import type { CoverageAxis, ConsumptionSample, PeriodStats, PeriodState, QuotaAccount, QuotaProvider, QuotaWindow, WindowAnalytics } from './types.js';
import { asRecord, consumptionSample, directQuota, num, periodStats, refreshState, str, windowAnalytics } from './types.js';

// 한 기간의 표본이 얼마나 닫혀 있는지. 값이나 구간 길이를 모르면 단정하지 않는다.
const TOL = 1 / 3600;
export function coverageAxis(value: unknown, hours: unknown): CoverageAxis {
  if (!finite(value) || !finite(hours) || hours <= 0) return 'unknown';
  if (value <= TOL) return 'none';
...
export function periodState(periods: unknown, key: string): PeriodState {
  ...
  const short = (axis: CoverageAxis) => axis === 'none' || axis === 'partial';
  ...
  if (observation === 'none') word = '미관측';
  else if (record === 'none' && !stats.requests) word = '기록 없음';
  else if (short(record) && short(observation)) word = (stats.observedCoverageHours ?? 0) <= (stats.logCoverageHours ?? 0) ? '관측 부족' : '기록 부족';
  else if (observation === 'partial') word = '관측 부족';
  else if (short(record)) word = '기록 부족';
  const spans: string[] = [];
  const span = (value: number | null) => number.format(value ?? 0) + '/' + number.format(hours ?? 0) + '시간';
  if (short(record)) spans.push('기록 ' + span(stats.logCoverageHours));
  if (short(observation)) spans.push('관측 ' + span(stats.observedCoverageHours));
  // 상태어를 붙이는 규칙은 행과 상세가 같다. 앞에 오는 문구만 다르다.
  const withWord = (base: string) => word === '기록 없음' ? word : word ? base + ' · ' + word : base;
...
export function freshWindow(account: QuotaAccount, window: QuotaWindow): boolean {
...
export function measurementReason(account: QuotaAccount, window: QuotaWindow): string {
...
export function lookupDelay(account: QuotaAccount): string | null {
  const refresh = refreshState(account.refresh);
  if (refresh?.status !== 'delayed' || ['reauth','paused'].includes(account.status)) return null;
...
export function usageAmount(stats: PeriodStats | null | undefined): string {
  if (!stats || !finite(stats.apiUsd)) return '—';
  return usd(stats.apiUsd);
}
// 행과 상세가 함께 쓰는 앞머리. 해당하는 상태가 없으면 null 이고, 거기서부터 두 문구가 갈린다.
function baseNote(stats: PeriodStats | null | undefined): string | null {
...
export function usageNote(stats: PeriodStats | null | undefined): string {
...
export function rowNote(stats: PeriodStats | null | undefined): string {
  const head = baseNote(stats);
  if (head) return head;
  if (!stats) return '기록 없음';
...
const tokenCount = (value: number) => value > 0 && count.format(value) === '0' ? '< 1' : count.format(value);
...
export function excludedNote(stats: PeriodStats | null | undefined, {lead = '단가 미확인', compact = false}: {lead?: string; compact?: boolean} = {}) {
  if (!stats || !(stats.unknownPriceRequests > 0)) return null;
  const parts = [`${lead} ${count.format(stats.unknownPriceRequests)}회`];
  if (stats.unknownPriceTokens > 0) parts.push(compact
    ? `기록된 ${tokenCount(stats.unknownPriceTokens)} 토큰 제외`
    : `기록된 ${tokenCount(stats.unknownPriceTokens)} 토큰이 합계에서 빠짐`);
  if (stats.unknownPriceUnsizedRequests > 0) parts.push(compact
...
export function cacheNote(stats: PeriodStats | null | undefined): string | null {
  if (!stats || !(stats.cacheEstimatedRequests > 0)) return null;
...
export function coverage(stats: PeriodStats | null | undefined, pace?: {pricedCoverage?: number} | null): number {
  if (!stats) return 0;
...
const DIRECT_STATUS: Record<string, string> = {
  ok: '조회 정상',
  partial: '일부 조회 실패',
  unknown: '조회 기록 없음',
...
export function directStatusNote(status: unknown): string | null {
...
export function directStatusIsWarning(status: unknown): boolean {
  return typeof status === 'string' && status !== 'ok';
}
// 계약이 null 을 허용하는 자리와, 값이 있지만 읽을 수 없는 자리를 구분해 적는다.
export function instantNote(value: unknown, format: (d: Date) => string): string {
...
const LIMIT_STATE: Record<string, string | null> = {present: null, zero: '한도 0', missing: '한도 미제공', unlimited: '한도 없음'};
export function limitStateNote(measurement: {limitState?: unknown} | null | undefined): string | null {
  const key = str(measurement?.limitState);
  return key ? LIMIT_STATE[key] ?? null : null;
}

const RECONCILIATION: Record<string, string> = {matched: '제공사 값과 계산값 일치',
  mismatch: '제공사 값과 계산값 불일치', unverified: '대조할 짝 없음'};
export function reconciliationNote(measurement: {reconciliation?: unknown} | null | undefined): string | null {
...
const PRECISION: Record<string, string> = {observed_fraction: '소수점 보고 확인',
  integer_only: '정수만 보고', unknown: '소수점 보고 여부 미확인'};
export function precisionNote(measurement: {precisionEvidence?: unknown} | null | undefined): string | null {
...
// 그 판단의 근거가 이 필드다.
const SEMANTICS: Record<string, string> = {fixed_reset: '고정 주기', sliding: '이동 구간', unknown: '주기 미확인'};
export function semanticsNote(measurement: {windowSemantics?: unknown} | null | undefined): string | null {
...
export function nextAttemptNote(account: QuotaAccount | null | undefined): string | null {
  const at = Date.parse(directQuota(account?.directQuota)?.nextAttemptAt ?? '');
  if (!finite(at)) return null;
  const wait = at - Date.now();
  if (wait <= 0) return '다음 조회 대기';
...
// 아니라 과거 읽기로 표시해야 한다.
export function withheldReading(account: QuotaAccount | null | undefined) {
...
const candidateWindow = (account: QuotaAccount | undefined, id: string) => account?.windows?.find(w => w.id === id) ?? null;
...
const unread = (account: QuotaAccount | undefined) => !(account?.windows?.length);
...
type AnalyzedWindow = QuotaWindow & { analytics?: WindowAnalytics };

function analyzed(w: QuotaWindow): AnalyzedWindow {
  return { ...w, analytics: windowAnalytics(w.analytics) };
}

export function weeklyQuotaUsage(provider: QuotaProvider | null | undefined, period = 'weekly') {
  const published = asRecord(asRecord(asRecord(provider?.analytics)?.quotaConsumption)?.[period]);
  if (published) return fromPublished(published, provider?.accounts ?? [], period);
  ...
  for (const id of QUOTA_BASIS) {
    const candidates = accounts.map(a => candidateWindow(a, id)).filter((w): w is QuotaWindow => w !== null).map(analyzed);
    const held = candidates.filter(w => w.analytics?.providerWide === true);
    if (held.length) return summariseQuotaUsage(held, id, accounts, period);
    if (candidates.some(w => w.analytics?.providerWide === undefined)) undeclared = true;
  }
  if (undeclared) {
...
const PERIOD_HOURS: Record<string, number> = {oneHour:1, fiveHour:5, twentyFourHour:24, weekly:168, monthly:720};

function summariseQuotaUsage(heldIn: AnalyzedWindow[], windowId: string, accounts: QuotaAccount[], period: string) {
  ...
  const held: AnalyzedWindow[] = heldIn.map(w => {
    const a = w.analytics ?? {};
  ...
  const recovered = measured.map(w => w.analytics?.selectedPeriodSample)
    .filter((sample): sample is ConsumptionSample => (sample?.recoveredHours ?? 0) > 0);
  ...
  const resetGaps = measured.reduce((sum, w) =>
    sum + (w.analytics?.selectedPeriodSample?.resetGapCount ?? 0), 0);
  const spanHours = mean(measured.map(w => finite(w.analytics?.forecastSpanHours) ? w.analytics.forecastSpanHours : 0));
  const seen = measured.map(w => w.analytics?.forecastCoverage).filter((v): v is number => finite(v));
  ...
  const others = accounts.filter(a => !(a?.windows ?? []).some(w =>
    w.id === windowId && windowAnalytics(w.analytics)?.providerWide === true));
...
function fromPublished(t: Record<string, unknown>, accounts: QuotaAccount[], period: string) {
