...
import type { PeriodKey, PeriodState, QuotaAccount, QuotaProvider, QuotaWindow, UiContext } from './types.js';
import type { ConsumptionSample, Snapshot, WindowAnalytics } from './types.js';
import { asPeriodKey, asRecord, consumptionMap, consumptionSample, directQuota, historicalCapacity, num, periodMap, periodStats, recommendationOf, refreshState, str, subscriptionInfo, windowAnalytics } from './types.js';

function requireNode<T extends HTMLElement>(id: string): T {
...
function winA(w: QuotaWindow): WindowAnalytics {
  return windowAnalytics(w.analytics) ?? {};
}

function accountPeriods(a: QuotaAccount) {
  return periodMap(asRecord(a.analytics)?.periods);
}

function providerPeriods(p: QuotaProvider) {
  return periodMap(asRecord(p.analytics)?.periods);
}

function parseOllama(value: unknown): {windows: {label: string; models: {model: string; inputTokensPerPp: number; outputTokensPerPp: number; apiUsdPerPp: number | null; requests: number}[]}[]} | null {
...
function sourcesOf(value: unknown): {url?: string; label?: string; checkedAt?: string}[] {
...
function markPeriod(item: HTMLElement, key: string, state: PeriodState, selectedPeriod: string | null, label: string) {
...
export function periodGrid(analytics: {periods?: unknown} | null | undefined, selectedPeriod: string | null) {
...
function detailPeriod(key: string, selectedPeriod: string, changePeriod: (period: PeriodKey) => void) {
...
function selectedRecommendation(provider: QuotaProvider, period: string) {
  const recs = asRecord(asRecord(provider.analytics)?.quotaRecommendations);
  return recs ? recommendationOf(recs[period]) : null;
}
export function recommendationView(rec: {status?: string; recommendedAccounts?: number | null; reason?: string; usageStale?: boolean} | null | undefined) {
  ...
  const r = rec as {totalConsumedPp?: number | null; sampleAccounts?: number; basisPeriod?: string; reason?: string} | null | undefined;
  ...
  } else if (rec?.reason) line.title = rec.reason;
  return line;
}
export function subscriptionNote(sub: {monthlyUsd?: number | null; label?: string; basis?: string} | null | undefined, ratio: unknown, basis?: string) {
...
export function windowAnalyticsView(w: QuotaWindow, key = w.id) {
  const a = winA(w);
  ...
  const capacity = metric('100% 한도', finite(value) ? `≈ ${usd(value)}` : '—',
    prior ? null : a?.capacityBasis === 'workload-estimate' ? '작업 비율 추정' : a?.capacityBasis === 'lower-bound' ? '보수 추정' : a?.capacityBasis === 'partial' ? '일부 관측' : null);
  ...
  if (!cycles) return grid;
  const wrap = node('div', 'capacity-block');
  wrap.append(grid, cycles);
  return wrap;
}
type CycleAnalytics = Pick<WindowAnalytics, 'capacityCycles' | 'capacityRangeApiUsd' | 'capacityShift'>;
const shortDate = {format(d: Date) { const p = (n: number) => String(n).padStart(2, '0');
...
export function capacityCyclesView(a: CycleAnalytics, expandKey: string): HTMLElement | null {
  ...
  const head = node('tr');
  for (const label of ['주기 끝', '100% 한도', '소모', '']) head.append(node('th', '', label));
  ...
  for (const c of newest.slice(0, CYCLE_ROWS)) {
    ...
    row.append(node('td', '', shortDate.format(new Date(c.to))), node('td', '', usd(c.apiUsd)),
      node('td', '', `${quotaFigure(c.matchedDeltaPp ?? c.deltaPp)}%p`), node('td', '', state));
    body.append(row);
  }
...
export function windowView(account: QuotaAccount, w: QuotaWindow, providerId = '') {
  ...
  item.append(quota);
  if (!['paused', 'reauth', 'unavailable'].includes(account.status)) item.append(windowAnalyticsView(analytics, `${providerId}:${account.id}:${w.id}`));
  return item;
}
export function accountView(provider: QuotaProvider, a: QuotaAccount, selectedPeriod: string, changePeriod: (period: PeriodKey) => void) {
  ...
  if (delay) {
    ...
    if (lastAttempt && finite(Date.parse(lastAttempt))) note.title += ` 마지막 조회 시도: ${date.format(new Date(lastAttempt))}`;
  ...
  if (directNote && dq) {
    ...
    if (dq.reason) badge.title = `사유: ${dq.reason}`;
  ...
  for (const w of a.windows) {
    ...
    if (wa.capacityReason || wa.reason) section.append(node('p','sub-note',wa.capacityReason || wa.reason || ''));
    if (finite(wa.capacityMatchedQuotaCoverage) && wa.capacityMatchedQuotaCoverage < 1) section.append(node('p', 'sub-note', `사용액이 연결된 쿼타 변화 ${quotaFigure(wa.capacityMatchedQuotaCoverage * 100)}%`));
    if (finite(wa.unexplainedDeltaPp) && wa.unexplainedDeltaPp > 0) section.append(node('p', 'sub-note', `환산 못한 쿼타 변화 ${quotaFigure(wa.unexplainedDeltaPp)}%p 포함`));
    ...
    if (finite(consumed)) section.append(node('p','sub-note', `${periodLabel(selectedPeriod)} ${sample?.basis === 'observed-increase' ? '관측 증가분 추정' : '소모'} ≈ ${quotaFigure(consumed)}%p` +
    ...
    if (finite(wa.forecastRatePpHour)) section.append(node('p','sub-note', `평균 ${quotaFigure(wa.forecastRatePpHour)}%p/시간 · ${number.format(wa.forecastObservedHours ?? 0)}시간 관측`));
  ...
  if (ollama) {
    ...
    let rows = 0;
    for (const w of ollama.windows) for (const m of w.models) {
      rows++;
...
export function providerAnalytics(p: QuotaProvider, ctx: UiContext & {changePeriod?: (period: PeriodKey) => void}) {
  ...
  if (finite(subMonthly)) wrap.append(node('p', 'sub-note', `현재 월 구독료 ${usd(subMonthly)} · 전체 등록 계정`));
  ...
  if (unpriced.length) wrap.append(node('p','sub-note',`최근 30일 미환산: ${unpriced.map(item => {
    const m = asRecord(item) ?? {};
  ...
  if (rec && finite(rec.currentAccounts)) details.append(node('p', 'sub-note', `현재 ${count.format(rec.currentAccounts)}개 계정`));
  ...
  for (const [id, label] of [['five-hour', '5시간 한도'], ['weekly', '주간 한도']] as const) {
  ...
  if (unattr && num(unattr.requests)) details.append(node('p','sub-note',`최근 7일 계정 미연결 ${usd(num(unattr.apiUsd))} (전체 합계에 포함)`));
  for (const {key, label} of PERIODS) {
...
export function topEfficiency(ctx: UiContext) {
  const notes = [
  ...
  if (catalog) notes.push(`등록된 기준 단가로 자동 계산하며, 없는 모델은 로컬 카탈로그의 동일 제공자·모델 단가를 사용합니다. 카탈로그 ${str(catalog.status) === 'ok' ? '정상' : str(catalog.status) === 'stale' ? '이전 자료 사용' : '조회 불가'}.`);
  ...
  if (finite(monthlySub)) efficiency.prepend(node('p', 'sub-note', `등록 계정 월 구독료 합계 ${usd(monthlySub)}`));
}
export function quotaOverview(p: QuotaProvider) {
  const groups = new Map<string, {label: string; fullLabel: string; remaining: number; accounts: number; lowest: number | null; reset: number | null}>();
  for (const a of p.accounts) for (const w of a.windows) {
    const key = `${w.id.startsWith('custom-') ? 'custom' : w.id}:${w.usageScope || ''}:${w.label}`;
    if (!groups.has(key)) groups.set(key, {label:windowLabel(w), fullLabel:w.label, remaining:0, accounts:0, lowest:null, reset:null});
    ...
    group.accounts++;
    if (group.lowest === null || value < group.lowest) group.lowest = value;
    const at = w.resetAt ? Date.parse(w.resetAt) : NaN;
    if (finite(at) && at > Date.now() && (group.reset === null || at < group.reset)) group.reset = at;
  }
  ...
  for (const g of groups.values()) {
    ...
    if (remaining === null) bar.append(node('p', 'quota-note', '최근 측정 없음'));
      ...
      const notes: string[] = [];
      if (g.accounts > 1 && g.lowest !== null && Math.abs(g.lowest - remaining) >= 1) notes.push(`최저 ${quotaFigure(g.lowest)}%`);
      if (g.reset !== null) notes.push(countdown(g.reset));
      if (notes.length) bar.append(node('p', 'quota-note', notes.join(' · ')));
    }
    values.append(bar);
  }
...
function quotaUsageCell(provider: QuotaProvider, period: string) {
...
export function overviewView(providers: QuotaProvider[], ctx: UiContext) {
  const wrap = node('div', 'overview');
  wrap.append(attentionStrip(ctx));
  wrap.append(summaryList(providers, ctx));
  return wrap;
}
function summaryList(providers: QuotaProvider[], ctx: UiContext) {
  ...
  if (!providers.length) list.append(node('div', 'empty', str(asRecord(ctx.snapshot?.analytics)?.status) === 'collecting'
    ? '저장된 사용 기록을 불러오고 있습니다.'
    : '연결된 제공자가 없습니다. OpenCodex에서 계정을 연결해 주세요.'));
  return list;
}
export function renderOrderEditor(providers: QuotaProvider[], ctx: UiContext) {
  ...
  providers.forEach((p, index) => {
    ...
    for (const [direction, label] of [[-1, '위로'], [1, '아래로']] as const) {
      ...
      button.addEventListener('click', () => {
        ...
        if (here === undefined || swap === undefined) return;
        ids[index] = swap;
        ids[other] = here;
...
export function basisRows(measurement: Record<string, unknown> | null | undefined) {
...
export function directQuotaBlocks(a: QuotaAccount) {
  const dq = directQuota(a.directQuota);
  if (!dq) return [];
  ...
  if (withheld) {
    ...
    if (!windows.length) section.append(node('p', 'unavailable', '보관된 읽기 없음'));
...
// ---- 계정 현황 · 비용 분석 -------------------------------------------------------------
// 서버가 analytics.accounts 와 analytics.costs 로 이미 정렬·집계해 보낸다. 여기서는 읽고
// 그리기만 한다. 금액은 모두 API 환산 참고값이며 청구액이 아니다.

type Health = 'critical' | 'warning' | 'ok' | 'unknown' | 'idle';
type CostCell = {requests: number; tokens: number; inputTokens: number; outputTokens: number; cachedTokens: number; apiUsd: number; unpricedRequests: number; tokenlessRequests: number; lastRequestAt: string | null};
type AccountWindowRow = {id: string; label: string; remainingPercent: number | null; resetAt: string | null; stale: boolean};
...
function n0(value: unknown): number { return num(value) ?? 0; }
function costCell(value: unknown): CostCell {
  const o = asRecord(value) ?? {};
...
function windowRow(value: unknown): AccountWindowRow | null {
  const o = asRecord(value);
  if (!o) return null;
...
export function accountRows(snapshot: Snapshot | null): AccountRow[] {
...
function costRows(value: unknown): CostRow[] {
  if (!Array.isArray(value)) return [];
...
export function countdown(at: number | string | null): string {
  const ms = (typeof at === 'string' ? Date.parse(at) : at ?? NaN) - Date.now();
  if (!finite(ms)) return '리셋 미제공';
  if (ms <= 0) return '리셋됨 · 새 측정 대기';
...
function ago(value: string | null): string {
  ...
  if (!finite(ms)) return '기록 없음';
  const m = Math.floor(ms / 60000), h = Math.floor(m / 60), d = Math.floor(h / 24);
  return m < 1 ? '방금' : m < 60 ? `${m}분 전` : h < 24 ? `${h}시간 전` : `${d}일 전`;
}
const compact = new Intl.NumberFormat('ko-KR', {notation:'compact', maximumFractionDigits:1});
function tokens(value: number): string { return value > 0 ? compact.format(value) : '0'; }
function money(value: number): string { return value > 0 ? usd(value) : '$0'; }

function healthBadge(health: Health): HTMLElement {
  const badge = node('span', `badge health-${health}`, HEALTH_LABEL[health]);
  return badge;
}
function miniBar(remaining: number | null, label: string): HTMLElement {
  const wrap = node('div', `mini-bar sev-${severityOf(remaining)}`);
  wrap.append(quotaTrack(remaining, label));
  return wrap;
}

export function attentionStrip(ctx: UiContext): HTMLElement {
  ...
  if (!accountRows(ctx.snapshot).length) { box.hidden = true; return box; }
  if (!rows.length) {
  ...
  for (const r of rows.slice(0, 6)) {
    ...
    top.append(node('span', 'attention-name', `${r.providerName} · ${r.label}`),
      node('strong', 'num', r.lowest?.remainingPercent != null ? `${quotaFigure(r.lowest.remainingPercent)}%` : '—'));
    item.append(top, node('span', 'attention-reason', r.reason));
    const reset = r.lowest?.resetAt ?? r.nextResetAt;
    if (reset) item.append(node('span', 'attention-reset', countdown(reset)));
    list.append(item);
  }
  box.append(list);
  return box;
...
export function accountsView(ctx: UiContext, query: string): HTMLElement {
  ...
  for (const h of ['critical', 'warning', 'ok', 'unknown', 'idle'] as Health[]) {
  ...
  if (!all.length) { section.append(node('div', 'empty', '계정 정보를 불러오는 중입니다.')); return section; }
  if (!rows.length) { section.append(node('div', 'empty', '일치하는 계정이 없습니다')); return section; }
  ...
  for (const label of ['계정', '상태', '가장 낮은 한도', '다음 리셋', '24시간', '7일', '최근 요청']) {
    const cell = node('span', '', label); cell.setAttribute('role', 'columnheader'); head.append(cell);
  }
  table.append(head);
  for (const r of rows) {
    const tr = node('div', `account-tr health-${r.health}`);
    tr.setAttribute('role', 'row');
    const cell = (cls: string, label: string) => { const c = node('div', `cell ${cls}`); c.setAttribute('role', 'cell'); c.dataset.label = label; tr.append(c); return c; };
    ...
    if (r.health !== 'ok') state.append(node('small', '', r.reason));
    ...
    if (r.lowest && r.lowest.remainingPercent !== null) {
      ...
      if (others.length) low.append(node('small', '', others.map(w => `${w.label} ${quotaFigure(w.remainingPercent)}%`).join(' · ')));
    } else low.append(node('span', 'muted', '—'));
...
const COST_PERIODS = [{key:'day', label:'24시간'}, {key:'week', label:'7일'}, {key:'month', label:'30일'}] as const;
...
export function costsView(ctx: UiContext): HTMLElement {
  ...
  if (!costs) { section.append(node('div', 'empty', '비용 집계를 불러오는 중입니다.')); return section; }
  ...
  if (period) {
    ...
    if (t.unpricedRequests) facts.push(`단가 미확인 ${count.format(t.unpricedRequests)}회 제외`);
    if (t.tokenlessRequests) facts.push(`토큰 미보고 ${count.format(t.tokenlessRequests)}회`);
...
function amounts(value: unknown): Record<string, number> {
  ...
  for (const [k, amount] of Object.entries(asRecord(value) ?? {})) { const x = num(amount); if (x !== null) out[k] = x; }
  return out;
}
function costSeries(value: unknown): CostSeries {
  const o = asRecord(value) ?? {};
...
function barRange(b: {from: string; to: string}, hours: number): string {
  const from = new Date(b.from), to = new Date(b.to);
  if (!finite(from.getTime())) return '';
  if (hours >= 24) return barDay.format(from);
...
// What one bar's detail says: a headline, a table of the stacked parts with
// the same columns in every row, and optional notes.
type DetailRow = {id: string; name: string; values: string[]};
type BarDetail = {summary: string[]; columns: string[]; rows: DetailRow[]; notes: string[]; empty?: string};

function detailNodes(range: string, d: BarDetail, colour: Map<string, string>): HTMLElement[] {
  const head = node('div', 'tip-head');
  head.append(node('strong', '', range));
  for (const s of d.summary) head.append(node('span', 'num', s));
  const out: HTMLElement[] = [head];
  if (d.empty) { out.push(node('p', 'tip-note', d.empty)); return out; }
  if (d.rows.length) {
    ...
    for (const r of d.rows) {
      ...
      const sw = node('span', 'swatch'); sw.style.background = colour.get(r.id) ?? '#888';
  ...
  for (const n of d.notes) out.push(node('p', 'tip-note', n));
  return out;
}
...
function stackedChart(opts: {bars: StackedBar[]; bucketHours: number; title: string; order: string[]; colour: Map<string, string>;
  ...
  for (const id of order.slice(0, 8)) {
    ...
    const sw = node('span', 'swatch'); sw.style.background = colour.get(id) ?? '#888';
  ...
  bars.forEach((d, i) => {
    ...
    if (!d.total || d.total <= 0) g.append(svg('rect', {x: String(i * bw + pad / 2), y: String(H - 1), width: String(Math.max(1, bw - pad)), height: '1', class: d.total === null ? 'zero missing' : 'zero'}));
  ...
  const reserve = () => {
    ...
    if (!tip.isConnected || width === measuredWidth) return;
    measuredWidth = width;
    ...
    if (activeIndex >= 0) show(activeIndex); else idle();
  };
...
function bucketChart(series: CostSeries, order: string[], colour: Map<string, string>, names: Map<string, string>, spanLabel: string): HTMLElement {
  ...
  return stackedChart({
    ...
    describe: i => {
      const b = buckets[i];
      if (!b) return {summary: [], columns: [], rows: [], notes: []};
      const share = (x: number) => b.apiUsd > 0 ? `${percent.format(x / b.apiUsd * 100)}%` : '';
...
function rankTable(title: string, rows: CostRow[], label: (r: CostRow) => string, colour: Map<string, string>, showRemoved: boolean): HTMLElement {
  const box = node('section', 'rank');
  box.append(node('h3', '', title));
  if (!rows.length) { box.append(node('p', 'muted', '기록 없음')); return box; }
  ...
  for (const r of rows) {
    ...
    if (r.share > 0) meta.unshift(`${percent.format(r.share * 100)}%`);
    if (r.unpricedRequests) meta.push(`단가 미확인 ${count.format(r.unpricedRequests)}회`);
    if (r.tokenlessRequests) meta.push(`토큰 미보고 ${count.format(r.tokenlessRequests)}회`);
...
let quotaSelection: CostKey = 'week';
const QUOTA_PERIOD_KEY: Record<CostKey, string> = {day: 'twentyFourHour', week: 'weekly', month: 'monthly'};

type QuotaBar = {from: string; to: string; deltaPp: number | null; byAccount: Record<string, number>; apiUsd: number; requests: number};

function quotaBars(value: unknown): {bucketHours: number; buckets: QuotaBar[]} {
  const o = asRecord(value) ?? {};
  const buckets = Array.isArray(o.buckets) ? o.buckets.flatMap(item => {
    const b = asRecord(item);
...
type QuotaProviderSeries = {provider: QuotaProvider; windowLabel: string; bars: QuotaBar[]; bucketHours: number;
  total: number | null; measured: number; held: number; need: number | null; apiUsd: number | null; accounts: Map<string, string>};

export function quotaView(ctx: UiContext): HTMLElement {
  ...
  const rows: QuotaProviderSeries[] = (ctx.snapshot?.providers ?? []).flatMap(provider => {
  ...
  section.append(head);
  if (!rows.length) { section.append(node('div', 'empty', '제공자 전체 주간·월간 한도가 있는 제공자가 없습니다.')); return section; }
  ...
  if (base && base.bars.length) {
    ...
    section.append(stackedChart({
      ...
      describe: i => {
        ...
        if (!present.length) return {summary: [], columns: [], rows: [], notes: [], empty: '이 구간은 관측이 없습니다. 사용량이 0이라는 뜻이 아닙니다.'};
  ...
  for (const id of order) {
    ...
    if (r.apiUsd !== null) facts.push(`API 환산 ${money(r.apiUsd)}`);
    if (r.total && r.apiUsd) facts.push(`1%p당 ${money(r.apiUsd / r.total)}`);
    if (r.need !== null) facts.push(`필요 계정 약 ${count.format(r.need)}개`);
  ...
  const accRows: {pid: string; id: string; label: string; pp: number; remaining: number | null; windowLabel: string}[] = [];
  for (const r of rows) {
    const sums = new Map<string, number>();
    for (const b of r.bars) for (const [id, x] of Object.entries(b.byAccount)) sums.set(id, (sums.get(id) ?? 0) + x);
  ...
  if (!accRows.length) accList.append(node('li', 'muted', '관측된 소모 없음'));
