...
function setDrawer(open: boolean, {restoreFocus = true}: {restoreFocus?: boolean} = {}) {
...
function select(id: string) {
  ...
  } else htmlEl($('providers').querySelector('.nav-item.active'))?.focus({preventScroll:true});
  window.scrollTo({top:0, behavior:'instant'});
}
function expandedKeys() {
  return new Set([...document.querySelectorAll('details[open][data-expand]')].flatMap(el => {
    const key = (el as HTMLElement).dataset.expand;
    return key ? [key] : [];
  }));
}
function restoreExpanded(keys: Set<string>) {
  for (const el of document.querySelectorAll('details[data-expand]')) {
    const key = (el as HTMLDetailsElement).dataset.expand;
    if (key && keys.has(key)) (el as HTMLDetailsElement).open = true;
  }
}
function orderedProviders(): QuotaProvider[] {
...
function render() {
  ...
  const entries: {id: string; name: string; accounts?: unknown[]; badge?: string; section?: string}[] = [
    {id:'overview', name:'요약'},
  ...
  for (const p of entries) {
    if (p.section) nav.append(node('span', 'nav-section', p.section));
    ...
    if (p.badge) b.append(node('span', 'attention', p.badge));
    else if (p.accounts) b.append(node('span', '', String(p.accounts.length)));
  ...
  for (const p of all) {
    ...
    const detailContext = (key: string) => ({ ...ctx,
      selectedPeriod: ctx.detailPeriods.get(key) ?? 'weekly' as const,
      changePeriod: (period: PeriodKey) => { ctx.detailPeriods.set(key, period); render(); },
    });
  ...
  if (!special && !countGroups) {
    ...
    clear.addEventListener('click', () => { requireEl<HTMLInputElement>('search').value = ''; select('all'); });
    empty.append(clear);
  ...
  if (focusedDetails) htmlEl([...document.querySelectorAll('details[data-expand]')].find(el=>htmlEl(el)?.dataset.expand===focusedDetails)?.querySelector('summary'))?.focus({preventScroll:true});
  if (focusedProvider) htmlEl([...nav.querySelectorAll('button')].find(el=>el.dataset.provider===focusedProvider))?.focus({preventScroll:true});
  if (focusedKey) htmlEl([...document.querySelectorAll('[data-focus]')].find(el => htmlEl(el)?.dataset.focus === focusedKey))?.focus({preventScroll:true});
...
async function refresh() {
  ...
  try {
    ...
    refreshStatus();
    requireEl<HTMLButtonElement>('refresh').disabled = false;
...
DRAWER_QUERY.addEventListener('change', () => {
  ...
  if (!DRAWER_QUERY.matches) {
    ...
    if (wasOpen || inDrawer || onToggle) htmlEl($('providers').querySelector('.nav-item.active'))?.focus({preventScroll:true});
  } else if (inDrawer) {
...
if (!PERIODS.some(period => period.key === ctx.selectedPeriod)) ctx.selectedPeriod = asPeriodKey(PERIODS[0]?.key ?? 'weekly');
