...
export function periodState(periods, key) {
  ...
  else if (short(record) && short(observation)) word = stats.observedCoverageHours <= stats.logCoverageHours ? '관측 부족' : '기록 부족';
  else if (observation === 'partial') word = '관측 부족';
  else if (short(record)) word = '기록 부족';
  const spans = [];
  const span = value => number.format(value) + '/' + number.format(hours) + '시간';
...
export function usageNote(stats) {
  return baseNote(stats) ?? (stats.unknownPriceRequests > 0 ? '일부' : `${count.format(stats.requests)}회 호출`) +
...
export function rowNote(stats) {
  return baseNote(stats) ?? `${count.format(stats.requests)}회 호출` + (stats.unknownPriceRequests > 0 ? ' · 일부' : '');
}
...
export function limitStateNote(measurement) {
  return LIMIT_STATE[measurement?.limitState] ?? null;
}
...
export function reconciliationNote(measurement) {
  return RECONCILIATION[measurement?.reconciliation] ?? null;
}
...
export function precisionNote(measurement) {
  return PRECISION[measurement?.precisionEvidence] ?? null;
}
...
export function semanticsNote(measurement) {
  return SEMANTICS[measurement?.windowSemantics] ?? null;
}

// 다음 조회 시점. account.refresh 쪽 lookupDelay 와 같은 모양으로 말한다.
export function nextAttemptNote(account) {
  const at = Date.parse(account?.directQuota?.nextAttemptAt);
...
export function withheldReading(account) {
  const lastKnown = account?.directQuota?.lastKnown;
...
export function weeklyQuotaUsage(provider, period = 'weekly') {
  ...
  for (const id of QUOTA_BASIS) {
    const candidates = accounts.map(a => candidateWindow(a, id)).filter(Boolean);
...
function summariseQuotaUsage(held, windowId, accounts, period) {
  const periodHours = {oneHour:1, fiveHour:5, twentyFourHour:24, weekly:168, monthly:720}[period];
  held = held.map(w => {
    const a = w.analytics, current = a.consumptionPeriods?.[period];
    const historical = !finite(current?.deltaPp) ? a.historicalConsumptionPeriods?.[period] : null;
    const sample = historical ?? current;
    return {...w, analytics:{...a,
      selectedHistoricalPeriod: historical, selectedPeriodSample: sample,
      forecastDeltaPp: sample ? sample.deltaPp : period === 'weekly' ? a.forecastDeltaPp : null,
      forecastSpanHours: sample ? sample.spanHours : period === 'weekly' ? a.forecastSpanHours : null,
      forecastCoverage: sample ? sample.coverage : period === 'weekly' ? a.forecastCoverage : null}};
  });
  const label = windowLabel(held[0]);
  ...
  const value = measured.reduce((sum, w) => sum + w.analytics.forecastDeltaPp, 0);
  const recovered = measured.map(w => w.analytics.selectedPeriodSample)
    .filter(sample => sample?.recoveredHours > 0);
  const recoveredPp = recovered.reduce((sum, sample) => sum + sample.recoveredDeltaPp, 0);
  ...
  const seen = measured.map(w => w.analytics.forecastCoverage).filter(finite);
  ...
  const others = accounts.filter(a => !(a?.windows ?? []).some(w =>
    w.id === windowId && w.analytics?.providerWide === true));
  ...
  return {
    ...
    value: '≈ ' + quotaFigure(value) + '%p' + (historical.length ? ' · 이전 관측' : ''),
    note: [label, n + '/' + m + '계정',
