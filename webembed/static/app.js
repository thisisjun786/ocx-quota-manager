...
import { accountView, providerAnalytics, overviewView, topEfficiency, renderOrderEditor, accountsView, costsView, quotaView, accountRows } from './views.js';
...
const ctx = {
    ...
    render: () => render(),
    select: (id) => select(id),
    clearSearch: () => { requireEl('search').value = ''; render(); },
};
let busy = false;
...
try {
    ...
    if (Array.isArray(saved))
        ctx.providerOrder = [...new Set(saved.filter(id => typeof id === 'string'))];
}
catch {
...
function setDrawer(open, { restoreFocus = true } = {}) {
    ...
    else if (restoreFocus && DRAWER_QUERY.matches)
...
function select(id) {
    ...
    if (DRAWER_QUERY.matches) {
        ...
        htmlEl($('providers').querySelector('.nav-item.active'))?.focus({ preventScroll: true });
    window.scrollTo({ top: 0, behavior: 'instant' });
}
function expandedKeys() {
    return new Set([...document.querySelectorAll('details[open][data-expand]')].flatMap(el => {
...
function render() {
    ...
    const focusedDetails = focused?.matches('summary') ? focused.parentElement?.dataset.expand ?? null : null;
    const focusedProvider = focused?.closest('#providers button') instanceof HTMLElement
        ? focused.closest('#providers button')?.getAttribute('data-provider') : null;
    ...
    nav.replaceChildren();
    const roster = accountRows(ctx.snapshot);
    const urgent = roster.filter(r => r.health === 'critical' || r.health === 'warning').length;
    const entries = [
        { id: 'overview', name: '요약' },
    ...
    for (const p of entries) {
        if (p.section)
            nav.append(node('span', 'nav-section', p.section));
        const b = node('button', ctx.selected === p.id ? 'nav-item active' : 'nav-item', p.name);
        b.dataset.provider = p.id;
        if (ctx.selected === p.id)
            b.setAttribute('aria-current', 'page');
        if (p.badge)
            b.append(node('span', 'attention', p.badge));
        else if (p.accounts)
            b.append(node('span', '', String(p.accounts.length)));
    ...
    const search = requireEl('search');
    const searchLabel = search.closest('label');
    if (searchLabel)
        searchLabel.hidden = overview || costsPage || quotaPage || collectionPage;
    ...
    if (toolbarTitle)
        toolbarTitle.textContent = overview ? '사용 현황' : accountsPage ? '긴급도 순' : costsPage ? 'API 환산 비용' : quotaPage ? '한도 사용 흐름' : '계정별 사용 현황';
    for (const b of $('period-picker').querySelectorAll('button'))
        b.setAttribute('aria-pressed', String(b.dataset.period === ctx.selectedPeriod));
    topEfficiency(ctx);
    const query = requireEl('search').value.trim().toLowerCase();
    const content = $('content');
    if (!(collectionPage && content.firstElementChild === collection.root))
        content.replaceChildren();
    if (overview)
        content.append(overviewView(all, ctx));
    if (accountsPage)
        content.append(accountsView(ctx, query));
    if (costsPage)
        content.append(costsView(ctx));
    if (quotaPage)
        content.append(quotaView(ctx));
    if (collectionPage) {
        collection.update(all);
        if (content.firstElementChild !== collection.root)
            content.append(collection.root);
    }
    ...
    for (const p of all) {
        if (special)
            break;
        if (ctx.selected !== 'all' && p.id !== ctx.selected)
            continue;
        ...
        if (!p.enabled)
            heading.append(node('span', 'disabled', '연결 비활성'));
        ...
        if (!filtered.length)
            list.append(node('p', 'account unavailable', '연결된 계정이 없습니다. OpenCodex에서 계정을 연결해 주세요.'));
        group.append(list);
        content.append(group);
        countGroups++;
    }
    if (!special && !countGroups) {
        const empty = node('div', 'empty', '일치하는 계정이 없습니다');
        const clear = node('button', '', '검색 초기화');
        clear.addEventListener('click', () => { requireEl('search').value = ''; select('all'); });
        empty.append(clear);
        content.append(empty);
    }
    const collected = str(asRecord(ctx.snapshot.analytics)?.lastCollectedAt);
    ...
    if (focusedDetails)
        htmlEl([...document.querySelectorAll('details[data-expand]')].find(el => htmlEl(el)?.dataset.expand === focusedDetails)?.querySelector('summary'))?.focus({ preventScroll: true });
    if (focusedProvider)
        htmlEl([...nav.querySelectorAll('button')].find(el => el.dataset.provider === focusedProvider))?.focus({ preventScroll: true });
    if (focusedKey)
        htmlEl([...document.querySelectorAll('[data-focus]')].find(el => htmlEl(el)?.dataset.focus === focusedKey))?.focus({ preventScroll: true });
    // 갱신으로 읽던 요소가 사라지면 보이는 탐색 항목으로 초점을 돌린다.
    if ((focusedDetails || focusedProvider || focusedKey) &&
        (!document.activeElement || document.activeElement === document.body)) {
        htmlEl([nav.querySelector('.nav-item.active'), $('title')]
...
async function refresh() {
    ...
    busy = true;
    refreshStatus();
    requireEl('refresh').disabled = true;
    $('refresh').setAttribute('aria-label', '새로고침 중');
    try {
        if (ctx.selected === 'collection')
            void collection.refresh();
        ...
        if (data.analytics?.status === 'error' || data.analytics?.usageStale)
            warnings.push('사용 기록 갱신 지연 · 이전 사용액 표시');
        ...
        if (ctx.snapshot)
            render();
        if (!ctx.snapshot)
            $('content').replaceChildren(node('div', 'empty', 'OpenCodex 연결을 확인해 주세요'));
    }
    finally {
        busy = false;
        nextRefreshAt = Date.now() + REFRESH_MS;
        refreshStatus();
        requireEl('refresh').disabled = false;
...
$('sidebar').addEventListener('blur', event => {
    ...
    if (DRAWER_QUERY.matches && !ctx.drawerOpen && !event.relatedTarget)
        $('menu-toggle').focus({ preventScroll: true });
}, true);
$('drawer-close').addEventListener('click', () => setDrawer(false));
$('drawer-scrim').addEventListener('click', () => setDrawer(false));
document.addEventListener('keydown', event => { if (event.key === 'Escape' && ctx.drawerOpen)
    setDrawer(false); });
...
if (!PERIODS.some(period => period.key === ctx.selectedPeriod))
    ctx.selectedPeriod = asPeriodKey(PERIODS[0]?.key ?? 'weekly');
...
document.addEventListener('visibilitychange', () => { if (!document.hidden)
    ...
    refreshStatus(); });
window.addEventListener('focus', () => { if (!document.hidden)
    refresh(); });
window.addEventListener('online', refresh);
window.addEventListener('offline', () => { refreshStatus(); if (ctx.snapshot) {
    render();
    $('notice').hidden = false;
    $('notice').textContent = '오프라인 · 이전 값을 표시합니다.';
} });
window.addEventListener('pageshow', event => { if (event.persisted)
    refresh(); });
setInterval(() => {
    if (!busy && !document.hidden && navigator.onLine && Date.now() >= nextRefreshAt)
        refresh();
    if (ctx.snapshot && !document.hidden && renderedMinute !== Math.floor(Date.now() / 60000))
        render();
    refreshStatus();
}, 1000);
const initial = decodeURIComponent(location.hash.slice(1));
if (initial)
    ctx.selected = initial;
